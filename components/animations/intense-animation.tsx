...
export function IntenseAnimation({ shape = "circle" }: { shape?: "circle" | "triangle" | "square" | "diamond" }) {
  ...
  useEffect(() => {
    ...
    const drawShape = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, opacity: number) => {
      ctx.fillStyle = `rgba(232, 149, 95, ${opacity})`
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-white">
      <div className="absolute inset-0 bg-gradient-to-br from-orange-100/20 via-amber-50/15 to-white blur-3xl scale-125" />
      <div className="absolute inset-0 bg-gradient-to-tr from-white via-transparent to-orange-100/20 blur-2xl scale-125" />
